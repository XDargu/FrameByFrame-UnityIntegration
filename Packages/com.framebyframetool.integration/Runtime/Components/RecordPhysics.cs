...
[FrameByFrameRecordingOption(FrameByFrameBuiltInRecordingOptions.Colliders, "Records SphereCollider, BoxCollider, and CapsuleCollider shapes.")]
[FrameByFrameRecordingOption(FrameByFrameBuiltInRecordingOptions.Collisions, "Records collision enter events and contact points.")]
[FrameByFrameRecordingOption(FrameByFrameBuiltInRecordingOptions.Physics, "Records physics data such as Rigidbody velocity.")]
public class RecordPhysics : MonoBehaviour
{
	...
	void Record()
	{
		if (FbFManager.IsRecordingOptionEnabled(FrameByFrameBuiltInRecordingOptions.Colliders))
		{
			EntityData entity = FbFManager.RecordEntity(targetObject);

			if (RecordCollider && sphereCollider)
			{
				PropertyGroup group = entity.AddGroup(FrameByFrameBuiltInRecordingOptions.Colliders);
				...
				group.AddSphere("SphereCollider", gameObject.transform.TransformPoint(sphereCollider.center), sphereCollider.radius * maxScale, color, FrameByFrameBuiltInRecordingOptions.Colliders, null, PropertyFlags.Collapsed);
			}
			...
			if (RecordCollider && boxCollider)
			{
				...
				group.AddOOBB("BoxCollider", boxCollider.bounds.center, size, gameObject.transform.up, gameObject.transform.forward, color, FrameByFrameBuiltInRecordingOptions.Colliders, null, PropertyFlags.Collapsed);
			}
			...
			if (RecordCollider && capsuleCollider)
			{
				...
				group.AddCapsule("CapsuleCollider", capsuleCollider.bounds.center, capsuleRef, capsuleCollider.radius, capsuleCollider.height, color, FrameByFrameBuiltInRecordingOptions.Colliders, null, PropertyFlags.Collapsed);
			}
		}

		if (FbFManager.IsRecordingOptionEnabled(FrameByFrameBuiltInRecordingOptions.Physics))
		{
			EntityData entity = FbFManager.RecordEntity(targetObject);
			...
			if (RecordVelocity && rigidBody)
			{
				...
				group.AddLine("Velocity", gameObject.transform.position, gameObject.transform.position + rigidBody.velocity, Color.blue, FrameByFrameBuiltInRecordingOptions.Physics, null, PropertyFlags.Collapsed);
			}
		}
	...
	{
		if (FbFManager.IsRecordingOptionEnabled(FrameByFrameBuiltInRecordingOptions.Collisions))
		{
			if (RecordCollider)
			{
				...
				{
					eventData.AddSphere("Contact", contact.point, 0.1f, Color.blue, FrameByFrameBuiltInRecordingOptions.Collisions);
